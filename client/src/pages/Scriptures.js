  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-8 flex flex-wrap gap-6 justify-center">
        ...
        <div className="flex items-center gap-2 text-gray-700">{statsLoading ? (<FiClock className="text-gray-400 animate-pulse" />) : (<svg className="w-5 h-5 text-orange-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>)}<span className="font-semibold">{statsLoading ? '...' : loadedVerses.toLocaleString()}</span><span className="text-gray-500">Verses Available</span><span className="text-xs text-gray-400 ml-1">({totalVerses ? pct + '%' : '—'} coverage)</span></div>
      </div>
          ...
          <div key={book.id} className={`rounded-xl border shadow-sm transition-all duration-200 ${isLoaded ? 'bg-white border-green-300 border-solid hover:shadow-md hover:border-green-400' : 'bg-gray-50 border-dashed border-gray-300'}`}>
            <div className="p-6">
              <div className="flex items-start justify-between mb-3">
                ...
                {isLoaded ? (
                  <span className="inline-flex items-center gap-1 text-xs font-semibold px-3 py-1.5 rounded-full bg-green-100 text-green-700 border border-green-300"><FiCheckCircle size={14} /> {stats[book.id].toLocaleString()} verses</span>
                ) : isFuture ? (
